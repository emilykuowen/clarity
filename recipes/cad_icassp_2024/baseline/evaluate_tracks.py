...
def run_calculate_aq(config: DictConfig) -> None:
    ...
    reference_folder = Path("reference_signals")
    ...
    for idx, scene_listener_pair in enumerate(scene_listener_pairs, 1):
        ...
        enhanced_signal, _ = read_flac_signal(
            Path(enhanced_folder) / f"{scene_id}_{listener_id}_remix.flac"
        )

        reference_mixture, _ = read_flac_signal(
            Path(reference_folder) / f"{scene_id}_{listener_id}_remix_ref.flac"
        )

        # Compute the scores
        left_score = compute_haaqi(
            processed_signal=resample(enhanced_signal[:, 0], config.remix_sample_rate, config.HAAQI_sample_rate),
            reference_signal=resample(reference_mixture[:, 0], config.remix_sample_rate, config.HAAQI_sample_rate),
        ...
        right_score = compute_haaqi(
            processed_signal=resample(enhanced_signal[:, 1], config.remix_sample_rate, config.HAAQI_sample_rate),
            reference_signal=resample(reference_mixture[:, 1], config.remix_sample_rate, config.HAAQI_sample_rate),
            processed_sample_rate=config.HAAQI_sample_rate,
            reference_sample_rate=config.HAAQI_sample_rate,
