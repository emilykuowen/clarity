...
from enhance import process_remix_for_listener, save_flac_signal
...
def generate_reference(config: DictConfig) -> None:
    ...
    for idx, scene_listener_pair in enumerate(scene_listener_pairs, 1):
        ...
        remix_reference_filename = Path(reference_folder) / f"{scene_id}_{listener_id}_remix_ref.flac"
        
        if remix_reference_filename.is_file():
            logger.info(
                f"{scene_id}_{listener_id}_remix_ref.flac already exists "
        ...
        logger.info(
            ...
            f"Generating reference {scene_id} for listener {listener_id}"
        )
        ...
        reference_signal = process_remix_for_listener(
            signal=reference_mixture,
            enhancer=enhancer,
            compressor=None,
            listener=listener,
